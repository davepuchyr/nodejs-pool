...
app.get('/config', function (req, res) {
    res.json({
        ...
        payout_period: Math.ceil( global.config.payout.timer / 60 ),
...
if (!global.argv.debug && cluster.isMaster) {
    let numWorkers = Math.max( require('os').cpus().length / 2, 1 );
